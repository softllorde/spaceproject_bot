...
class Database_Manager:
	...
	def add_user(self, id, name):
		if not self.checkUserInDataBase(id):
			...
			self.cursor.execute("INSERT INTO player_spaceships (player_id, spaceship_id, attack, defence, hold_capacity) VALUES(?, ?, ?, ?, ?)", (id, 1, 1, 1 , 5))
			self.conn.commit()
	...
	def get_player_spaceship(self, id):
		...
		if spaceship:
			player_id,spaceship_id,name,type,attack,defence,hold_capacity = spaceship
		else:
			return None
